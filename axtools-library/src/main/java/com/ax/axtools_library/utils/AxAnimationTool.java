...
import com.ax.axtools_library.interfaces.OnDoIntListener;

public class AxAnimationTool {
...
//                textView.setTextColor((Integer) animation.getAnimatedValue());
    ...
    public static void cardFilpAnimation(final View beforeView, final View AfterView) {
        ...
        if (beforeView.getVisibility() == View.GONE) {
            ...
            AxImageTool.invisToVis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", -90f, 0f);
            AxImageTool.visToInvis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", 0f, 90f);
        } else if (AfterView.getVisibility() == View.GONE) {
            AxImageTool.invisToVis = ObjectAnimator.ofFloat(AfterView,
                    "rotationY", -90f, 0f);
            AxImageTool.visToInvis = ObjectAnimator.ofFloat(beforeView,
                    "rotationY", 0f, 90f);
        }

        AxImageTool.visToInvis.setDuration(250);// 翻转速度
        AxImageTool.visToInvis.setInterpolator(accelerator);// 在动画开始的地方速率改变比较慢，然后开始加速
        AxImageTool.invisToVis.setDuration(250);
        AxImageTool.invisToVis.setInterpolator(decelerator);
        AxImageTool.visToInvis.addListener(new Animator.AnimatorListener() {

            @Override
            public void onAnimationEnd(Animator arg0) {
                if (beforeView.getVisibility() == View.GONE) {
                    AfterView.setVisibility(View.GONE);
                    AxImageTool.invisToVis.start();
                    beforeView.setVisibility(View.VISIBLE);
                } else {
                    AfterView.setVisibility(View.GONE);
                    AxImageTool.visToInvis.start();
                    beforeView.setVisibility(View.VISIBLE);
                }
            }
